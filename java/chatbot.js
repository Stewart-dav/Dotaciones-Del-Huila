...
function coincidePalabraConNombre(nombreNormalizado, palabra) {
    ...
    return nombreNormalizado.split(/\s+/).some(token => {
        ...
        return raizToken === raizBuscada
            || raizToken.startsWith(raizBuscada)
            || raizBuscada.startsWith(raizToken);
    });
}
...
function buscarProductoPorNombre(textoNormalizado) {
    const palabras = textoNormalizado.split(/\s+/).filter(w => w.length >= 4 && !PALABRAS_RELLENO.has(w));
    if (palabras.length === 0) return [];

    return inventario
        .filter(p => {
            const nombreNorm = normalizarTexto(p.nombre);
            return palabras.some(w => coincidePalabraConNombre(nombreNorm, w));
        })
        .slice(0, 8);
}
...
function procesarConsulta(mensaje) {
    ...
    if (categoriasSolicitadas.length > 0) {
        ...
        const palabrasFiltro = texto
            .split(/\s+/)
            .filter(w => w.length >= 4 && !PALABRAS_CATEGORIA_TODAS.has(w) && !PALABRAS_RELLENO.has(w));

        let respuestaFinal = "";
        categoriasSolicitadas.forEach(item => {
            respuestaFinal += consultarStockCategoria(item.id, item.label, filtroPrecio, palabrasFiltro) + "<br><br>";
        });
    ...
    if (coincidencias.length > 0) {
        const bloques = coincidencias.map(p => {
            const precioMostrar = formatearPrecio(p);
            return `• <b>${p.nombre}</b><br>${precioMostrar}`;
        });
        return `Encontré estas coincidencias en el catálogo:<br><br>` + bloques.join("<br><br>");
    }
...
function consultarStockCategoria(cat, nombreMostrar, filtroPrecio, palabrasFiltro) {
    ...
    let encabezado = `Tenemos las siguientes opciones en ${nombreMostrar}:<br><br>`;
    ...
    if (palabrasFiltro && palabrasFiltro.length > 0) {
        ...
        if (filtrados.length > 0) {
            productos = filtrados;
            encabezado = `Esto es lo que tengo en ${nombreMostrar} que coincide con "${palabrasFiltro.join(" ")}":<br><br>`;
        } else {
            encabezado = `No encontré coincidencias exactas con "${palabrasFiltro.join(" ")}", pero esto es todo lo disponible en ${nombreMostrar}:<br><br>`;
        }
    }
    ...
    return encabezado + bloques.join("<br><br>");
}
...
function formatearPrecio(producto) {
    ...
    if (!precioActual) return "Precio a consultar";
    ...
    if (producto.precioAnterior && producto.precioAnterior.trim() !== "" && producto.precioAnterior.trim() !== precioActual) {
        // Dato cargado a mano en datos.js: manda sobre cualquier otra fuente,
        // porque es igual para todos los visitantes.
        precioAnteriorMostrar = producto.precioAnterior.trim();
    } else if (precioVistoAntes && precioVistoAntes !== precioActual) {
        // Este navegador ya había visto un precio distinto en una visita anterior.
...
const COMANDOS_LIMPIAR = [
    ...
    "reiniciar chat", "limpiar conversacion", "borrar conversacion"
];
